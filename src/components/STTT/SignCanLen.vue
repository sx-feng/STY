  ...
  <div class="sign-card">
    <div class="sign-head">
      ...
      <div class="meta">
        连续天数：<b>{{ streakDays }}</b>
        <span class="dot">•</span>
        本月断签：<b :class="{ danger: missedDays > maxMiss }">{{ missedDays }}</b> / {{ maxMiss }}
      </div>
    </div>

    <!-- 日历网格 -->
    <div class="calendar">
      <div class="wday" v-for="w in ['一','二','三','四','五','六','日']" :key="w">{{ w }}</div>
      <div
        v-for="cell in cells"
        :key="cell.key"
        class="cell"
    ...
    <div class="actions">
      ...
      <span class="tip" v-if="locked">已超过 {{ maxMiss }} 次断签，本月签到已断档</span>
      <span class="tip" v-else-if="isTodaySigned">今天已签到</span>
    </div>
  </div>
</template>
...
/** ===== 本月签到数据（localStorage） ===== */
const signedSet = ref(new Set()) // 存 'YYYY-MM-DD'
...
onMounted(load)
watch(signedSet, persist, { deep: true })
...
.title { font-size: 16px; font-weight: 600; color: #ffd34d; }
...
.cell.is-empty { background: transparent; border: none; }
.day { position: relative; display: flex; align-items: center; gap: 6px; }
.num { font-weight: 600; color: #e8eaed; }
.mark { color: #22c55e; font-weight: 700; }
.miss { color: rgba(255,255,255,.28); font-size: 20px; line-height: 1; margin-top: -2px; }
.cell.is-today { border-color: rgba(255,211,77,.7); box-shadow: inset 0 0 0 1px rgba(255,211,77,.35); }
.cell.is-signed { background: rgba(34,197,94,.12); border-color: rgba(34,197,94,.35); }
.cell.is-missed { background: rgba(255,255,255,.02); }
...
.tip.danger, .meta b.danger { color: #ff6b6b !important; }
</style>
